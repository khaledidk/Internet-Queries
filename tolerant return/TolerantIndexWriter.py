...
class TolerantIndexWriter:
    ...
    def __init__(self, in_dir, out_dir):
        ...
        word_dic = read.words_dic

        files = {"reviews.tbl", "products.dic.fxd", "texts.dic.str", "collection.data" , "bigram.lst" , "bigram.dic"}
    ...
    def startFile(self, Out_Dir, In_Dir, file, data):  # starting all files
        ...
        if file == "texts.dic.str":
            ...
            f.close()
        elif file == "bigram.dic":
            pointer= 0
            file = os.path.join(Out_Dir, file)
            f = open(file, "w")
            bigram_dic = self.convert_all_token_to_bigram(data)
            print("bigram_dic" , bigram_dic)
            for bigram in bigram_dic:
               WriteTable = bigram + ' ' + str(pointer) + '\n'
               pointer += len(str(bigram_dic[bigram]) + ' ')

               f.write(WriteTable)
        elif file == "bigram.lst":
            file = os.path.join(Out_Dir, file)
            f = open(file, "w")
            bigram_dic = self.convert_all_token_to_bigram(data)
            for bigram in bigram_dic:
              WriteTable = str(bigram_dic[bigram]) + ' '
              f.write(WriteTable)
    ...
    def convert_all_token_to_bigram(self , data):
        list_of_bigrams = []
        bigrams_dic_by_ids = {}
        bigrams_dic_by_word = {}

        word_id = 1
        for token in data.words_dic:
            list_for_token = self.convert_token_to_bigram(token[0])
            for bigram in list_for_token:
                if(bigram not in  list_of_bigrams  ):
                  list_of_bigrams.append(bigram)
                  bigrams_dic_by_word[bigram]=[]
                  bigrams_dic_by_word[bigram].append(token)

                  bigrams_dic_by_ids[bigram] = []
                  bigrams_dic_by_ids[bigram].append(word_id)
                elif(bigram in  list_of_bigrams and word_id not in bigrams_dic_by_ids[bigram]) and token not in bigrams_dic_by_word[bigram]:
                    bigrams_dic_by_word[bigram].append(token)
                    bigrams_dic_by_ids[bigram].append(word_id)
            word_id+=1

        # print("real" , bigrams_dic_by_word)
        return bigrams_dic_by_ids
